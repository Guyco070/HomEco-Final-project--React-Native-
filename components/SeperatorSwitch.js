...
const SeperatorSwitch = ({isExpended, setIsExpended, title, topDevider, bottomDevider, width, titleColor, withCheckIcon}) => {
  return (
    <View style={{width:"95%",...styles.container}}>
      <TouchableOpacity style={[styles.touchableOpacity, {borderTopWidth: topDevider ? 1 : 0, borderBottomWidth: bottomDevider ? 1 : 0, width: width ? width : "100%"}]} onPress={()=>{ setIsExpended(!isExpended) }}>
        <View style={{flexDirection:"row", marginTop:topDevider ? 20 : 0}}>
          {withCheckIcon && <Icon style={styles.checkIcon} name= { !isExpended ? 'ellipse-outline' : "checkmark-circle-outline"} size={22}  type='ionicon' />}
          <Text style={{color: titleColor? titleColor : "black"}}>{title}</Text>
        </View>
